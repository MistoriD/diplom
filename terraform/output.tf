...
[bastionhost]
bastion-host ansible_host=${yandex_compute_instance.bastion-host.network_interface.0.nat_ip_address} ansible_ssh_user=shmaginmy
...
[webservers:vars]
ansible_ssh_user=shmaginmy
ansible_ssh_common_args='-o ProxyCommand="ssh -W %h:%p shmaginmy@${yandex_compute_instance.bastion-host.network_interface.0.nat_ip_address}"'
